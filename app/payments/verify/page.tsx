...
export default function PaymentVerifyPage() {
  ...
  const verifyPayment = async (reference: string) => {
    try {
      ...
      // Handle both cases: new verification (with tickets) and already verified (without tickets)
      if (response.tickets && response.tickets.length > 0) {
        setTicketIds(response.tickets.map(t => t.id));
      } else {
        // If no tickets in response, it means payment was already verified
        // We can still show success but without specific ticket IDs
        setTicketIds([]);
      }
    } catch (error) {
      setStatus('failed');
